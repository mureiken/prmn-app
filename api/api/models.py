...
class User(Updateable, db.Model):
    ...
    role = sqla.Column(sqla.String(10), index=True)
...
class Subscriber(db.Model):
    ...
    id = sqla.Column(sqla.Integer, primary_key=True)
    email = sqla.Column(sqla.String(120), index=True,
                        nullable=False)
    organisation = sqla.Column(sqla.String(120), index=True,
                        nullable=False)
    region = sqla.Column(sqla.String(120), index=True,
                        nullable=False)
    subscription_date = sqla.Column(sqla.DateTime, index=True, default=datetime.utcnow,
                            nullable=False)
    report_type = sqla.Column(sqla.String(25), index=True,
                        nullable=False)
    daily = sqla.Column(sqla.Boolean, default=True,
                        nullable=False)
    weekly = sqla.Column(sqla.Boolean, default=True,
                        nullable=False)
    active = sqla.Column(sqla.Boolean, default=True,
                        nullable=False)
    
    __table_args__ = (
        sqla.UniqueConstraint(email, report_type),
    )
